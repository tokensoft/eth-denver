...
contract TokenRegulatorService is RegulatorService, Ownable {
  ...
  // @dev Check error reason: Receiver is not allowed to receive the token due to AML/KYC
  uint8 constant private CHECK_ERECV_AML_KYC = 5;

  // @dev Check error reason: Receiver is not allowed to receive the token due to Geography
  uint8 constant private CHECK_GEOGRAPHY = 6;
  ...
  /// @dev Permission bits for allowing a participant to receive tokens if they are AML/KYC
  uint8 constant private PERM_RECEIVE_AML_KYC = 0x4;

  /// @dev Permission bits for allowing a participant to receive tokens if they are in a country that supports the token
  uint8 constant private PERM_RECEIVE_GEOGRAPHY = 0x8;
  ...
  function getPermission(address _token, address _participant) public view returns (uint8) {
  ...
  function check(address _token, address _spender, address _from, address _to, uint256 _amount) public returns (uint8) {    
    ...
    if (settings[_token].enforceAmlKYC && (participants[_token][_to] & PERM_RECEIVE_AML_KYC == 0)) {
      return CHECK_ERECV_AML_KYC;
    }

    if (settings[_token].enforceGeography && (participants[_token][_to] & PERM_RECEIVE_GEOGRAPHY == 0)) {
      return CHECK_GEOGRAPHY;
    }
